...
namespace View {
    ...
    public:
        ...
        std::shared_ptr<EntityModel> createGhost(const Vector2D& position, double wait_delay, int color_index) override;
        std::shared_ptr<EntityModel> createPacman(const Vector2D& position) override;
